...
public:
	...
	int analysis(double bid, double ask) {
		...
		for (int i = 0; i < flow->tradeFilter.size(); i++) {
			if (flow->tradeFilter.at(i).size > (averageOrder * 2)) {
				...
				if ((char)*flow->tradeFilter.at(i).aggrSide.pData == 'B') {
					toSupp->push_back(zone(flow->tradeFilter.at(i).price, flow->tradeFilter.at(i).time));
				...
				else if ((char)*flow->tradeFilter.at(i).aggrSide.pData == 'S') {
					toResist->push_back(zone(flow->tradeFilter.at(i).price, flow->tradeFilter.at(i).time));
	...
	int confirmation(double bid, double ask) {
		for (int i = 0; i < toSupp->size(); i++) {
			if (!toSupp->at(i).confirmed && toSupp->at(i).pulledAway && ((toSupp->at(i).price - bid) * (toSupp->at(i).price - bid ) < 1))
